...
## Replacement DEV turn 20 structured-root and key facts — July 26, 2026

- Turn 19's recursive object/list value traversal remains authoritative, but its root-type and actual-key identity boundaries were incomplete. Valid JSON string roots now use the same recursive nested-diagnostic and recognized escape-layer sanitizer, and changed roots are re-encoded as valid JSON strings instead of collapsing to unquoted `<redacted>`.
- Actual object-key spelling remains byte-for-byte token-preserved, but its decoded identity is now classified through recognized JSON escape layers to a fixed point. Every layer must be nonempty, printable, and no longer than 256 characters. Secret-equivalent identities redact only the associated complete JSON value; malformed, unknown, control-bearing, non-convergent, over-bound, or over-depth identities fail closed for the diagnostic.
- The shared structured depth limit remains 12. Six immediate/remaining Authorization, Proxy-Authorization, and Set-Cookie string-root cases, six root escape depths, twenty-four Authorization/Proxy-Authorization/Cookie/Set-Cookie key-depth cases, safe root/key controls across six depths, root/key over-depth cases, malformed remaining-key escapes, durable state, and CLI JSON stdout/stderr are covered.
- Direct diagnostics, nested serialization, `Failure`, valid state/stdout JSON, invariant exit `20`, empty stderr, original key spelling, safe siblings, and ordinary authorization/cookie business controls are preserved. The correction remains confined to redaction and public persistence/output boundaries.

## Replacement DEV turn 21 native-mapping key facts — July 26, 2026

- Native Python `Mapping` string keys now reuse the same strict `_canonical_json_key_is_secret()` fixed-point classifier as actual JSON text object keys before the associated value is recursively serialized or persisted. The original mapping key spelling remains unchanged.
- Secret-equivalent Authorization, Proxy-Authorization, Cookie, and Set-Cookie keys are covered through six recognized Unicode escape depths. Malformed, unknown, control-bearing, over-bound, non-convergent, and over-depth string-key identities fail closed for only their associated value while safe siblings remain visible.
- Native non-string mapping keys preserve the existing coercion contract: they are converted with `str()`, sanitized as output keys, and classified directly with the normalized secret predicate. Integer keys remain visible; an object stringifying to `authorization` still redacts its associated value.
- Direct `redact()`, `safe_json_dumps()`, and the real `StateStore._atomic_json()` persistence primitive are covered. The correction changes only the shared redaction boundary; transport, identity, graph, monitor, state machine, ownership, locking, helper lifecycle, cancellation, backend access, and CLI architecture are unchanged.

## Replacement DEV turn 22 final structured-boundary facts — July 26, 2026

- Every native Python `Mapping` key now follows the same path after coercion: `str(raw_key)` is retained as the output-key source and is also passed through the strict canonical fixed-point classifier. Custom objects that stringify to escaped Authorization, Proxy-Authorization, Cookie, or Set-Cookie identities therefore redact their associated values; malformed, control-bearing, over-bound, and over-depth coerced identities fail closed. Ordinary numeric and safe stringified keys remain visible.
- Canonical JSON object-key layers are checked both as secret labels and as bounded plain-text diagnostics. If a key token itself contains Authorization/Proxy-Authorization header values, Cookie/Set-Cookie material, Bearer/Basic credentials, JWT-like text, or a secret assignment, the complete structured diagnostic returns `<redacted>` because associated-value replacement cannot make the preserved key token safe. Legal authorization decisions, marketing cookie settings, authorization status, cookie policy, and basic settings controls remain visible.
- Structured-looking strings are classified before truncation. Inputs above `4 * max_length` no longer become invalid sliced JSON and fall through to unstructured sanitation. Over-budget JSON object/list roots return `<redacted>`; JSON string roots return valid JSON `"<redacted>"`. Boundary and large-form tests cover objects, lists, string roots, JSON-in-JSON, and nested arrays with canaries at the start, middle, and end, including repeated sanitation stability.
- Direct diagnostics, nested serialization, `Failure`, `StateStore._atomic_json()` and turn state, CLI JSON stdout/stderr, and exact state reload are covered. The correction remains confined to redaction and serialization; transport, identity, graph, monitor, state-machine, ownership, locking, helper lifecycle, cancellation, backend access, and CLI architecture are unchanged.

## Replacement DEV turn 23 final native-key output fact — July 26, 2026

- Native string and `str()`-coerced mapping keys that contain credential material behind recognized escape layers now emit the safe key/value pair `<redacted>: <redacted>` instead of preserving the unsafe key token. If that placeholder collides with another output key, the whole mapping collapses to the same single fail-closed pair.
- The bounded change is confined to the existing `Mapping` branch. Authorization, Proxy-Authorization, Cookie, Set-Cookie, access-token, proof-token, and JWT cases at escape depths 1–6 are covered for native strings, custom key objects, direct serialization, and atomic persistence. Safe mappings and siblings remain unchanged when no collision exists.
