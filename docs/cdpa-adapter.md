...
- Schema/invariant failure: block and require code repair/review. Unknown or missing nested identity fields are corrupt state, not forward-compatible metadata to be silently dropped. Adapter diagnostics must pass through the core sanitizer before persistence or display; JSON/Python-style quoted secret keys and explicit `Cookie:`/`Set-Cookie:` headers are not safe logging context.
