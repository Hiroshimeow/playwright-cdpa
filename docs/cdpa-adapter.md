...
- Ambiguous or identity-missing outcome: retain request identity and route to exact watch/recovery, never resend. For an uncertain existing-conversation click, the adapter must preserve the core baseline record unchanged; it must not rebuild, weaken, or discard baseline fingerprints, sibling branches, or ownership evidence to force recovery. Multiple post-baseline user deltas are an operational ambiguity, not a reason to select the current branch.
- Schema/invariant failure: block and require code repair/review. Unknown or missing nested identity fields are corrupt state, not forward-compatible metadata to be silently dropped. Adapter diagnostics must pass through the core sanitizer before persistence or display; JSON/Python-style quoted secret keys, escaped quotes inside secret values, and explicit `Cookie:`/`Set-Cookie:` headers with any HTTP-token cookie name are not safe logging context.
