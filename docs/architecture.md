...
- The shared coordination namespace defaults to the normalized loopback CDP endpoint. `localhost`, `127.0.0.1`, and `::1` aliases for the same scheme and port converge on one namespace. The coordination base is resolved to an absolute path during configuration validation. A relative `XDG_STATE_HOME` is not interpreted relative to process CWD; the core falls back to the OS account home only when that home is already absolute. A relative fallback home is invalid and fails before persistence or browser construction. Embedders may provide an explicit deployment ID and an absolute coordination base. Explicit relative coordination bases are invalid and fail before any persistence or browser operation, preventing repository-CWD-dependent ownership planes.
...
- credential-bearing URL userinfo, sensitive query parameters, fragments, and secret-bearing path segments. Secret labels are normalized across snake_case, kebab-case, camelCase, and compact forms. The shared predicate covers generic `*_token`, `*_secret`, `*_password`, `*_passwd`, `*_api_key`, `*_credential`, and `*_credentials` families plus bounded private/signing/encryption-key, secret-access-key, and passphrase forms, as well as known access/refresh/resume/session/proof/Sentinel/Turnstile labels. A credential marker joined to payload in one path segment causes the whole segment and following path context to be redacted; explicit documentation/resource suffixes and ordinary near-matches remain visible. Free-form secret assignments accept bounded unquoted keys and matching single- or double-quoted JSON/Python-style keys before `:` or `=`. Quoted keys are scanned independently from values, decode valid JSON-style escapes, accept the same printable bracket/punctuation/whitespace canonicalization as real mapping keys through the shared 256-character bound, and fail closed on malformed escapes, control characters, mismatched quotes, or over-bound assignment keys. If the matching close quote is absent, every raw `:` and `=` position on the current physical line is treated as a plausible assignment boundary because those characters may also be internal qualified-key separators. Candidate prefixes are evaluated longest-to-shortest with the same quote-specific canonical decoder and the raw bound implied by the 256-character decoded limit. Any decoded secret identity, undecodable candidate, or over-bound candidate redacts the complete diagnostic; context is preserved only when every plausible candidate is valid and nonsecret. Signature, vendor signed-URL signature, OAuth `code_verifier`, and `client_assertion` families are normalized across snake_case, kebab-case, camelCase, and compact forms. Explicit Authorization and Proxy-Authorization headers are handled before assignment scanning: redaction consumes the complete physical line and every immediately following SP/HTAB-prefixed continuation line regardless of scheme, then preserves the next non-continuation line. Structured mapping keys and matching quoted assignment keys classify exact and bounded qualified authorization, proxy-authorization, cookie, cookies, and set-cookie identities across separator, snake_case, kebab-case, camelCase, and compact forms. Separated qualifiers and lowercase compact compounds both use the approved request/response/network/direction/proxy/HTTP/header vocabulary; the complete prefix must be accepted, while ordinary suffix-sharing fields such as `marketing_cookie` and `legal_authorization` remain visible. Compact compounds additionally require an explicit `header` or `headers` component; existing one-level exact qualifiers remain supported. Unrecognized components never qualify merely because the key ends with a secret-looking suffix. Unquoted values consume until an explicit comma, semicolon, ampersand, newline, independently recognized following assignment, or end of input. Whitespace alone is part of the secret value, which prevents multi-word passphrase tails from surviving. Quoted values use an escape-aware grammar: each backslash plus following character is consumed as content, so escaped quotes and arbitrary backslash runs cannot terminate the value early; the true matching quote remains the boundary around `<redacted>`. If no true closing quote exists, the secret assignment consumes the remainder of the diagnostic fail-closed. A diagnostic that validates as a JSON string, object, or list is processed through one bounded structured boundary rather than the generic quoted-assignment scanner. A root string is decoded with `json.loads`, passed through the recursive nested-diagnostic and recognized JSON-escape-layer sanitizer, and re-encoded as a valid JSON string only when changed. Object/list roots use a token-preserving JSON-grammar traversal that identifies actual object keys and complete value spans, recursively enters object/list values, and treats array strings only as values. Actual decoded JSON-text key identities and the coerced text of every native Python `Mapping` key are canonicalized through the same recognized JSON escape layers to a fixed point; every layer must remain nonempty, printable, and at most 256 characters. Canonicalization is classification-only for safe and secret-identity keys. Each canonical layer is also evaluated by the bounded plain-text secret-boundary predicate. For native mappings, malformed, unknown, control-bearing, non-convergent, over-depth, or secret-bearing key tokens replace both the output key and value with `<redacted>`; an output-key collision collapses the mapping to `{`<redacted>`: `<redacted>`}`. Serialized JSON diagnostics with credential material in a key token fail closed completely because preserving the original token would disclose it. Non-string mapping keys preserve `str()` coercion before the same classification. Secret object keys replace exactly their associated JSON value; nonsecret string values are decoded with `json.loads`, recursively sanitized when they contain another structured diagnostic, and checked through bounded legal JSON-escape layers until a fixed point or the shared depth limit of 12. Only changed value spans are re-encoded. Structured-looking inputs are identified before any length slicing. If the raw string exceeds the bounded structured input budget (`4 * max_length`), it fails closed directly rather than being converted into invalid JSON and routed through unstructured fallback; JSON string roots retain valid JSON form as `"<redacted>"`, while object/list roots return `<redacted>`. This preserves valid JSON, key order, whitespace, punctuation, and safe array siblings for accepted bounded inputs while supporting Unicode, escaped slash, escaped quote, and backslash sequences. Query parameters already sanitized by the URL boundary are not expanded into following prose. Explicit `Cookie:` and `Set-Cookie:` headers use the RFC-style HTTP token alphabet for cookie names and are fully redacted before assignment parsing, including names with punctuation such as `+`, `$`, `!`, `^`, `|`, and `~`; multi-pair raw cookie blobs use the same name grammar and remain fully redacted. A single arbitrary `name=value` string without explicit cookie provenance is retained as bounded diagnostic context rather than guessed to be a cookie.
